...
function generateApiKey(){fetch('/api/external/keys/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:'Tool User'})}).then(function(r){return r.json();}).then(function(d){if(d.api_key){document.getElementById('tool-api-key').value=d.api_key;setApiKey();document.getElementById('key-status').textContent='New API Key generated: '+d.api_key;}});}
...
function searchDailymotion(){if(!apiKey){alert('Set API Key dulu!');return;}var search=document.getElementById('dm-search').value;var sort=document.getElementById('dm-sort').value;var limit=document.getElementById('dm-limit').value;var url='/api/dailymotion?limit='+limit+'&sort='+sort;if(search)url+='&search='+encodeURIComponent(search);fetch(url,{headers:getHeaders()}).then(function(r){return r.json();}).then(function(d){renderVideos(d,'dm-result');});}
function getBittv(){if(!apiKey){alert('Set API Key dulu!');return;}var type=document.getElementById('bittv-type').value;fetch('/api/keyrafa/bittv?type='+type,{headers:getHeaders()}).then(function(r){return r.json();}).then(function(d){renderEvents(d,'bittv-result');});}
function getMoviebox(){if(!apiKey){alert('Set API Key dulu!');return;}var type=document.getElementById('moviebox-type').value;var endpoint=type==='horror'?'/api/keyrafa/moviebox-horror':'/api/keyrafa/moviebox-global';fetch(endpoint+'?page=1&perPage=10',{headers:getHeaders()}).then(function(r){return r.json();}).then(function(d){renderMovies(d,'moviebox-result');});}
function getGempa(){if(!apiKey){alert('Set API Key dulu!');return;}fetch('/api/keyrafa/gempa?type=auto',{headers:getHeaders()}).then(function(r){return r.json();}).then(function(d){var container=document.getElementById('tool-result');var gempa=d.result&&d.result.gempa?d.result.gempa:[];if(!gempa||gempa.length===0){container.innerHTML='<p style="color:#666;">Tidak ada gempa terkini.</p>';return;}var html='<div class="video-grid">';for(var i=0;i<Math.min(gempa.length,10);i++){var g=gempa[i];html+='<div class="video-card fade-in">';html+='<h3>'+(g.wilayah||'Unknown')+'</h3>';if(g.magnitude)html+='<div class="link">Magnitude: '+g.magnitude+'</div>';if(g.kedalaman)html+='<div class="link">Kedalaman: '+g.kedalaman+' km</div>';if(g.waktu)html+='<div class="link">'+g.waktu+'</div>';html+='</div>';}html+='</div>';container.innerHTML=html;});}
function getTikTok(){if(!apiKey){alert('Set API Key dulu!');return;}var username=prompt('Masukkan username TikTok:');if(!username)return;fetch('/api/keyrafa/tiktok?username='+encodeURIComponent(username),{headers:getHeaders()}).then(function(r){return r.json();}).then(function(d){document.getElementById('tool-result').innerHTML='<pre>'+JSON.stringify(d,null,2)+'</pre>';});}
function showTranslate(){if(!apiKey){alert('Set API Key dulu!');return;}var text=prompt('Masukkan teks yang mau diterjemahkan:');if(!text)return;fetch('/api/keyrafa/translate?text='+encodeURIComponent(text)+'&to=en&from=auto',{headers:getHeaders()}).then(function(r){return r.json();}).then(function(d){document.getElementById('tool-result').innerHTML='<pre>'+JSON.stringify(d,null,2)+'</pre>';});}
function showScreenshot(){if(!apiKey){alert('Set API Key dulu!');return;}var url=prompt('Masukkan URL website:');if(!url)return;fetch('/api/keyrafa/ssweb?url='+encodeURIComponent(url),{headers:getHeaders()}).then(function(r){return r.json();}).then(function(d){var container=document.getElementById('tool-result');if(d.result&&d.result.url){container.innerHTML='<div class="video-card"><img src="'+d.result.url+'" alt="Screenshot" style="max-width:100%;border:3px solid #000;border-radius:8px;"></div>';}else{container.innerHTML='<pre>'+JSON.stringify(d,null,2)+'</pre>';}});}
var alightEmail='',alightLink='';
function sendAlightLink(){if(!apiKey){alert('Set API Key dulu!');return;}alightEmail=document.getElementById('alight-email').value.trim();if(!alightEmail||alightEmail.indexOf('@')===-1){alert('Masukkan email yang valid!');return;}document.getElementById('alight-status').textContent='Mengirim link ke '+alightEmail+'...';fetch('/api/alight/send',{method:'POST',headers:getHeaders(),body:JSON.stringify({email:alightEmail})}).then(function(r){return r.json();}).then(function(d){if(d.status){document.getElementById('alight-status').textContent='Link sent to '+alightEmail+'! Cek inbox/spam.';document.getElementById('alight-link').style.display='inline-block';document.getElementById('alight-activate-btn').style.display='inline-block';document.getElementById('alight-result').innerHTML='<p style="color:#4ECDC4;">'+d.message+'</p>';}else{document.getElementById('alight-status').textContent='Gagal: '+(d.error||d.message||'Unknown error');}});}
function activateAlight(){if(!apiKey){alert('Set API Key dulu!');return;}alightLink=document.getElementById('alight-link').value.trim();if(!alightLink||alightLink.indexOf('http')===-1){alert('Masukkan OOB link yang valid!');return;}document.getElementById('alight-status').textContent='Mengaktivasi premium...';fetch('/api/alight/activate',{method:'POST',headers:getHeaders(),body:JSON.stringify({email:alightEmail,link:alightLink})}).then(function(r){return r.json();}).then(function(d){if(d.status){document.getElementById('alight-status').textContent='Premium activated!';var html='<div style="background:#4ECDC4;padding:16px;border:3px solid #000;border-radius:10px;">';html+='<h3 style="color:#000;">Premium Activated!</h3>';html+='<p>Email: '+alightEmail+'</p>';if(d.premium){html+='<p>Status: '+(d.premium.active?'Active':'Inactive')+'</p>';if(d.premium.expiry)html+='<p>Expiry: '+new Date(d.premium.expiry).toLocaleString()+'</p>';html+='<p>Auto Renew: '+(d.premium.auto_renew?'Yes':'No')+'</p>';}html+='</div>';document.getElementById('alight-result').innerHTML=html;}else{document.getElementById('alight-status').textContent='Gagal aktivasi: '+(d.error||d.message||'Unknown error');}});}
